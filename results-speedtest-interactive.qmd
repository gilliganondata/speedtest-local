...
# Load key events log
df_key_events <- read_csv("key_events_log.csv") |>
  mutate(date = ymd(date),
         datetime_noon = as.POSIXct(paste(date, "12:00:00"), tz = "UTC"),
         event = str_wrap(event, width=20))
...
dateRangeInput("date_range", "Date Range", 
               start = as.Date(max(df_results_long$start_time))-30,
...
sliderInput("rolling_avg", "Rolling Avg. (Periods)", min = 1, max = 50, value = 1)
...
output$ts_plot <- renderPlot({
  ...
  # Filter key events to the selected date range
  df_events_in_range <- df_key_events |>
  ...
  gg <- ggplot(df_results_mbps, aes(x = start_time, y = rolling_avg)) +
    ...
    {if (nrow(df_events_in_range) > 0)
        geom_vline(data = df_events_in_range,
                   aes(xintercept = as.numeric(datetime_noon)),
                   color = "firebrick", linewidth = 0.6, linetype = "dashed")} +
    {if (nrow(df_events_in_range) > 0)
        geom_label(data = df_events_in_range,
                  aes(x = datetime_noon + 3800, y = Inf, label = event),
                  label.size = 0, color = "firebrick", size = 4, hjust = 0, vjust = 1.3,
                  inherit.aes = FALSE)} +
    scale_x_datetime(date_breaks = "1 day", date_labels = "%m/%d") +
    scale_y_continuous(expand = c(0,0), limits = ~ c(0, ifelse(nrow(df_events_in_range)==0, 
                                                                    max(.x)*1.08, max(.x)*1.4))) +
...
output$outtages <- render_gt({
  
  # Get the rows involved in outtages in the selected date range
  ...
  # If no outtages, then make a simple table
  if(nrow(df_outtages)){
  
  df_outtages <- df_outtages |> 
  ...
  gt(df_outtages) |> 
    ...
    df_no_outtages <- tibble(Outtage = "None Detected") |> 
      gt() |> 
    tab_options(
      table.align = "left"  # Left-justify entire table
    )
